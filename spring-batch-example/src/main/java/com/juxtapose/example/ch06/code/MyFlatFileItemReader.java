package com.juxtapose.example.ch06.code;
 ...
 * {@link #setRecordSeparatorPolicy(RecordSeparatorPolicy)} and mapped to item using {@link #setLineMapper(org.springframework.batch.item.file.LineMapper)}.
 ...
 */
public class MyFlatFileItemReader<T> extends AbstractItemCountingItemStreamItemReader<T> implements
        ResourceAwareItemReaderItemStream<T>, InitializingBean {

    private static final Log logger = LogFactory.getLog(org.springframework.batch.item.file.FlatFileItemReader.class);
    ...
    private String[] comments = new String[] { "#" };//定义注释行的前缀

    private boolean noInput = false;//当文件中存在需要跳读的时候用到，初始值是false,然后根据linesToSkip来确定是否置为ture

    private String encoding = DEFAULT_CHARSET;//读取文件的默认编码

    private LineMapper<T> lineMapper; //将一行文件记录转换为Java对象
    ...
    private boolean strict = true;//定义读取文件不存在的策略

    //根据给定的resource创建BufferReader实例，默认使用DefaultBufferedReadFactory创建文本类型的BufferReader实例
    private BufferedReaderFactory bufferedReaderFactory = new DefaultBufferedReaderFactory();

    public MyFlatFileItemReader() {
        setName(ClassUtils.getShortName(org.springframework.batch.item.file.FlatFileItemReader.class));
    }
